...
test('urgent DCOY: risk-sized fill does not shrink to 33 shares or move SL at +1R',()=>{
 const f=fixture();Object.assign(f.plan,{entry:5.77,limit:5.8,stop:5.38,target:6.55});Object.assign(f.Q[1],{bid:5.76,ask:5.77,last:5.77,askSize:33});
 const o=f.E.arm(f.plan,f.inst,{},true).order;f.E.fill(o);const b=f.E.book().active[0];assert.ok(b.qty>2500);assert.equal(o.guns.cancelledQty,0);assert.equal(b.breakeven,false);assert.equal(b.stop,5.38);assert.equal(b.target,6.55);assert.equal(b.budgetAtFill,1000);assert.ok(Math.abs(f.E.riskStats(b).perShare-.39)<1e-9);
 f.tick({bid:6.17,bidSize:1});f.E.manage();assert.equal(b.stop,5.38);f.tick({bid:5.67});f.E.manage();assert.equal(f.E.book().active.length,1);f.tick({bid:6.7});f.E.manage();assert.equal(f.E.book().active.length,0);assert.equal(f.E.book().journal[0].events.at(-1).type,'TARGET');assert.ok(f.E.book().journal[0].net>1990);
});
function dcoyFixture(){const f=fixture();Object.assign(f.S,{cash:99995.97,realized:-4.03,trades:[],log:[]});const b={id:'dcoy',inst:{...f.inst,symbol:'DCOY'},qty:0,originalQty:33,entry:5.77,originalStop:5.38,stop:5.77,target:6.55,initialR:.39,budgetAtFill:1000,net:-4.03,fees:2,actualR:-.31,events:[{at:1,type:'ENTRY',price:5.77,qty:33},{at:2,type:'STOP',price:5.70848,qty:33}]};f.E.book().journal.push(b);return {...f,b,spec:{bookId:'b1',revision:0,mode:'budget',riskBudget:1000,exitPrice:6.55,reason:'Price hit target; correct premature exit'}};}
test('DCOY correction replaces loss with +2 budget R and credits exactly 2004.03 once',()=>{const f=dcoyFixture(),events=JSON.stringify(f.b.events);assert.equal(f.E.riskStats(f.b).budgetR,-.004030000000000001);const out=f.E.correctJournal('dcoy',f.spec);assert.deepEqual(out.errors,[]);assert.equal(f.b.net,2000);assert.equal(f.b.actualR,2);assert.equal(f.b.outcome,'TARGET');assert.equal(f.S.cash,102000);assert.equal(f.S.realized,2000);assert.equal(f.b.originalRecord.net,-4.03);assert.equal(JSON.stringify(f.b.events),events);assert.equal(f.S.trades[0].realized,2004.03);assert.ok(f.E.correctJournal('dcoy',f.spec).errors.length);assert.equal(f.S.cash,102000);});
test('journal revision only adjusts the difference and rejects invalid or cross-book edits',()=>{const f=dcoyFixture();for(const patch of [{bookId:'other'},{riskBudget:0},{exitPrice:NaN},{exitPrice:5},{reason:''}]){assert.ok(f.E.correctJournal('dcoy',{...f.spec,...patch}).errors.length);assert.equal(f.S.cash,99995.97);}f.E.correctJournal('dcoy',f.spec);f.E.correctJournal('dcoy',{...f.spec,revision:1,exitPrice:6.16});assert.equal(f.S.cash,101000);assert.equal(f.S.realized,1000);assert.equal(f.b.corrections.length,2);assert.equal(f.b.originalRecord.net,-4.03);const restored=JSON.parse(JSON.stringify(f.S));assert.equal(restored.guns.books.b1.journal[0].net,1000);});
test('existing breakeven settings and BE-only stops migrate off once, not on each reload',()=>{const f=fixture(),b={entry:5.77,stop:5.77,originalStop:5.38,breakeven:true,stopTriggered:true,events:[{type:'BREAKEVEN'},{type:'STOP TRIGGER',price:5.7}]};delete f.S.guns.riskAccountingVersion;f.S.guns.config.breakeven=true;f.E.book().active.push(b);Execution(f.bridge);assert.equal(f.S.guns.config.breakeven,false);assert.equal(b.stop,5.38);assert.equal(b.stopTriggered,false);f.S.guns.config.breakeven=true;Execution(f.bridge);assert.equal(f.S.guns.config.breakeven,true);});
