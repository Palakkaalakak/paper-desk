...
## Portfolio modes, Trading preview and journals — 2026-09-24

This section supersedes the older deferred-work notes. Paper Desk remains a **local Python / vanilla-JavaScript workstation**, not a cloud deployment. Run `python serve.py` and open http://localhost:8765. `ib_async==2.1.0`, the dedicated IB owner loop, authentication, `readonly=True`, order-method seals and broker-write restrictions are unchanged. Even a live-account Gateway is used for data only; all execution is simulated in the browser.

### Completed controls
- **Portfolios:** choose GUNS, Trading or Custom when creating a portfolio. Legacy portfolios migrate to GUNS without resetting balances/history. Mode, workspace settings, managed positions and journals are isolated per portfolio. Switching/resetting is blocked while managed exposure remains.
- **Trading:** select a stock in the ordinary ticket or click its chart, then press **1** (or Prepare selected stock). Enter an entry price; blank SL uses **1 ATR(14)** of the selected **1m / 5m / 15m / daily** timeframe. Only completed broker candles count; incomplete/stale/mismatched data produces an error, not an invented ATR. SL rounds down to the tick. Automatic TP defaults to 2 price R and is configurable; explicit SL/TP are supported.
- **TP later:** choose “Entry + SL only; set TP later.” After filling, use **Set / edit TP** on the managed position. This may close immediately if a fresh executable bid already reaches that target. No automatic breakeven is added to Trading.
- **Preview:** update the preview after edits; **Enter confirms**, **Esc cancels**. Changed portfolio/selection or sizing requires rejection/review. Whole-share Trading sizing includes current equity risk %, entry/SL distance, estimated fees and buying power. The assisted preview is **long-stock-only**; existing ordinary manual tickets remain available for other supported sides/instruments.
- **Custom:** manually enter whole-share quantity and optional SL/TP. No automatic ATR, levels or quantity, and no chart request merely to prepare a Custom preview.
- **Entry behavior:** LMT waits for executable ask at/below the selected maximum. STPLMT additionally waits for an actual trade at/above entry and latches that trigger. Managed entries require fresh live Gateway quotes and positive ask size; they never use historical candles to fill. Full-size paper fills are not capped to the first displayed size and are not a real-liquidity guarantee. Trading quantity may shrink at fill for current risk/buying power, never increase beyond preview. Confirmed levels stay fixed; stops can slip at the bid, while targets fill at the saved TP when bid reaches it.
- **Strategy tags:** the workspace/default and preview tag propagate to new Trading/Custom orders, fills and lifecycle journals. GUNS retains its setup labels.

### Post-exit observations and detailed journals
- Configure **1–30 minutes** in the workspace panel (GUNS → Journal for GUNS) or order preview. Applies to new closes, not a reconstruction of historical trades.
- **History → Detailed trade journal** groups new non-GUNS, non-combo-leg fills from entry to flat, with additions, partial exits, reversals, fees, strategy, original/current levels and available risk metadata. GUNS results and their existing correction audit are included; GUNS → Journal also shows tracking directly on its own cards without duplicate journals. Unknown historical opening fills/fees/risk stay unknown. A new general non-GUNS correction editor is not implemented.
- Tracking continues across portfolio switches while the browser and Gateway run. It samples distinct live quote timestamps (bid for longs, ask for shorts), range, favorable/adverse movement, additional **price R excluding fees**, and later observed TP. It changes no cash, realized P&L or trade outcome. Missing intervals, stale streams, sleeping tabs/reloads and horizon expiry are reported; “TP not observed” is not proof it never hit. Observations have lower subscription priority than active execution.

### Premarket high and order fields
S1 records the highest actual **bar wick high** in the same-day premarket window (04:00 ET to the regular open for a normal US session), including an observed forming premarket bar. Overnight, regular-session and future bars are excluded. Preview records/shows the source bar/time/count and refreshes stale chart inputs. Automatic S1 entry is that high plus $0.01, tick-rounded; an explicit valid hovered-bar override is separately labeled. **ENTRY / SL / TP** are shown separately. The legacy stop-limit `order.stop` is the buy-entry trigger, **not** the protective SL; explicit aliases and consistency guards protect this mapping. The exact cause of the previously reported historical trade is not established.

### Storage, routes and remaining acceptance
State remains in `localStorage['paperAccount']`: `account.mode`, active `desk={settings,active,journal}`, inactive `books[].data.desk`, and per-journal `tracking`. No new storage service, credentials or broker-write route was added. `/` serves the workstation; authenticated `/assets/trading.js` and `/assets/trading-ui.js` serve the new modules. ATR uses the existing `/data/guns_bars?provider=tws&symbol=...`; scanner routes remain unchanged.

Keep browser, Python server and Gateway running: brackets are not broker-held and cannot protect a sleeping/closed browser. Next acceptance is live Gateway data/entitlements and timing on the user's machine; offline tests do not establish live reliability. Short/derivative assisted ATR brackets, native TWS embedding and continuous ATR trailing are not implemented. Source checkpoints do not back up the user's separate browser account.

### Final offline regression verification — 2026-09-24

- `node --test test_frontend.cjs test_guns.cjs test_trading.cjs`: **95 passed**.
- `python -m unittest test_market test_security`: **72 passed**.
- `python -u browser_check.py`, `python -u browser_trading_check.py`, and `python -u browser_guns_check.py`: **all passed, zero browser errors**. Coverage includes the full GUNS suite, tutorial financial/order isolation while legitimate tracking continues, Trading/Custom preview and fills, TP later, cross-book tracking, persistence and mobile layout.
- These are offline fixture regressions, not live Gateway acceptance. Update with `git pull --ff-only origin main`, restart Python and reload the same browser at http://localhost:8765.
...
- **Quick-load scanner stocks** (header and directly above Trading charts) explicitly assigns the visible candidates. Click any chart to select its stock; selection never trades. Intraday charts shade **04:00 New York time to the reported regular open** in translucent white, including partial 5m/15m overlaps and DST. Daily charts are unshaded; unknown sessions are not guessed.
- **1–5 opens a preview, Enter confirms, Esc cancels.** Shortcuts and S buttons never place an order before confirmation. The calculated preview captures the selected stock and reviewed levels and shows shares, entry, limit, stop, target and risk. Confirmation arms a paper stop-limit entry, not an instant fill. Repeated Enter cannot duplicate that submission; account changes are rejected and changed sizing requires another review.
- **S1–S3 calculate automatically from the same IB chart candles, including before 09:28.** No manual price, shares, review checkbox or fresh premarket quote is required to queue a calculable plan. S1 uses PM high, S2 the completed lower pivot, S3 the final completed flag candle. Stop settings, current paper equity, fees and buying power supply the bracket and whole-share sizing. Missing chart data is fetched; missing patterns/ATR/session or invalid numbers are reported without fabricating prices or forcing a manual ticket.
- **Confirmed S1–S3 orders wait for regular open.** Premarket quotes cannot trigger, fill or cancel them. Confirmed entry/SL/TP stay fixed across chart updates. At/after open, executable live IB quotes and displayed liquidity drive paper fills. A stop-limit above its cap waits, rather than chasing; once triggered it stays triggered until fill, user cancellation or session-end DAY expiry. Size follows current paper equity at fill; brackets protect only filled shares. Older saved pending orders retain their original policy; cancel/re-preview to use the new one.
- **Manual fills are optional, never the shortcut fallback.** **Manual paper BUY / SELL** is always available for the selected stock, including without live quotes. Positive finite prices and whole-share quantities are required; this long-stock ticket sells held shares, not shorts. Optional BUY brackets require both stop and target, or neither. Buying-power/strategy judgments do not veto this explicit path. Assumed fills may produce negative paper cash.
- Only explicit manual assumed fills record `USER_ENTERED_PAPER`; automatic fills remain quote-driven. Confirmed calculated plans save `confirmedPlan`, prices and advisory warnings in `paperAccount`. Quotes are never fabricated or altered. Screening judgments do not veto confirmed plans; optional Level II cancellation only affects unfilled entries. Protective exits remain active when executable live quotes exist; manual closing remains available without them. Brackets are browser-local, never broker-held.
...
71 Python tests and 70 JavaScript tests passed in the latest unit run. The expanded GUNS browser suite passed with zero browser errors, including manual BUY/SELL without quotes and beyond buying power, ledger provenance, reserves, quick-load, shading, S1–S5 and Level II. The general browser regression also passed with zero browser errors, including 500 positions, ordinary market/limit fills, stale/disconnected-data protections, settlement, export and mobile layout. Both browser suites and all 141 unit tests were rerun successfully on 2026-09-17 with the shortcut preview implementation. The expanded browser checks also passed shortcut-only/no-order, Esc cancellation, Enter confirmation, repeated Enter, captured hover levels, account-change rejection, changed-quantity review, mobile preview, Enter-confirmed manual fills, chart-click selection without orders and both quick-load locations.
...
- Restored the local **Scanner data setup / FMP key** form. `POST /data/guns_credentials` requires the existing authorization, a loopback client, JSON and the settings header; it saves `PAPER_FMP_KEY` into the ignored local `.env` atomically and activates it immediately. The UI clears the field and never saves the key in the browser account. A successful save is not a provider/coverage test. HTTP 402 symbol restrictions remain unresolved.
- Selected-strategy blockers are now visible as a complete list; each S button shows its own reasons. No numerical gates were removed to make buttons clickable.
- Parallel scanning preserves discovery order: quotes (6), cheap gap/price/spread/traded-volume stages, history verification (3 with three-second start pacing), then float (3) and refreshed finalists. Missing FMP configuration no longer aborts all IB stages.
- Persistent SMART Level II monitoring for S4/S5 uses at most three streams. Stale/incomplete depth holds entries. Confirmed displayed-liquidity flags hold for a decision or optionally auto-cancel unfilled entries only. Open-position stops and targets remain independent. Qualification and update callbacks now reject old Gateway generations; eviction cannot return an obsolete depth book.
- Added offline regressions for depth stream reuse, unchanged polling freshness, eviction, resets/reconnect generations, local credential authorization, atomic private file writes and UI key handling. The 60 Python and 59 JavaScript tests passed on 2026-09-15. Both existing browser suites passed with zero browser errors, including the FMP form and visible blockers. The expanded GUNS browser suite also completed successfully: off-tab Level II monitoring, missing-depth hold, confirmed review, five-second resume expiry, explicit cancellation and auto-cancel all passed, followed by tutorial/mobile and S5 checks with zero browser errors. The run exposed and fixed a Level II settings panel that collapsed on re-arm; its open state now survives rendering. These are offline fixture results, not live Gateway/entitlement acceptance.

**Unchanged architecture and storage:** Python `serve.py` / `market.py`, pinned `ib_async==2.1.0`, one IB owner loop, vanilla JavaScript and the browser `paperAccount` model. Source: https://github.com/Palakkaalakak/paper-desk on `main`; app: http://localhost:8765. No cloud deployment, cloud collector, broker-held stop, or real broker execution. New data access includes `/data/depth?symbol=...` and `/data/guns_capabilities`; runtime capability fields alone do not establish usable float counts.

**Still not established:** universal dated float coverage, live entitlement/Gateway acceptance and TWS chart parity. Continuous ATR trailing and native TWS embedding are not implemented. Automatic breakeven at +1R remains available, before fees. Next operational step is the read-only live-data acceptance checklist in the guide, not real-money trading.

## Prior GUNS workflow — 1.5 (2026-09-12; later entries below)
...
- **IBKR stream ownership fix (2026-09-14):** reproduced with the real `ib_async==2.1.0` wrapper: a transient quote request and browser streaming subscription for the same conId shared a ticker; the transient cleanup cancelled the newer persistent request, while the engine still considered it active. One-off reads now use the separate snapshot request channel and cancel their own request ID only. Timeout/cancellation/reconnect cleanup cannot cancel another request. Existing fresh IB quotes are reused. Scanner recovery now reads `/data/guns_ib_quote?conid=...` directly on the IB owner loop before trying any external source, so a browser SSE delivery problem no longer automatically rejects every quote. This is a reproduced application defect, not a claim that it was the only cause on the user's unobserved live Gateway.

- **Scanner quote acquisition (2026-09-14):** bounded parallel acquisition checks discovered candidates and refreshes finalists; browser subscriptions and direct owner-loop quote recovery are kept separate from external screening fallback. After history, after fundamentals, and before finalist publication, the scanner waits for a fresh LIVE positive bid, positive ask, uncrossed market and actual last trade. A zero bid, missing side, delayed/halted quote or mismatched contract is acquisition-pending, not a completed spread rejection. Actual wide spreads report the measured dollar amount and configured limit. Cards now show the verified **bid, ask, spread and quote timestamp**. Every restored/published snapshot must contain complete numeric price/gap/volume/spread and dated float evidence plus session/classification; incomplete legacy snapshots are discarded and an empty shortlist triggers initial acquisition. Screening snapshots remain fixed; loss of a current price shows the labeled screened price instead of a blank metric. No synthetic spread, default zero, delayed-data promotion, or broker orders were introduced. Missing source data can still prevent publication; the scanner does not invent it.

- **Live strategy tooltips (2026-09-13):** Trading → Plan & risk now shows context for the selected company and S1–S5. Hover the Setup selector or live summary for a native tooltip, or expand **Strategy tooltip & live levels** with mouse, keyboard or touch. S1 reports last-trade distance below/at/above the actual PM high in dollars and percent of that high (for example, `$0.21 (2.00%) below premarket high $10.50`). S2 reports pivot distance and resistance room in R; S3/S4 report flag-high distance and heuristic retracement; S5 reports the first minute's direction, range/ATR and remaining entry window. Details include planned entry distance, ask-to-limit headroom, formation and stop rules. Hover/manual overrides are explicitly distinguished from the actual PM high. Stale/mismatched chart data or a missing/non-live trade suppress live proximity; no midpoint substitute is used. Tooltips are informational and never arm, approve or change orders. Existing routes and storage are unchanged.
...
- **Float source correction (2026-09-14):** IBKR's official API 10.47 release notes remove `reqFundamentalData` and its callbacks. The scanner no longer waits on `ReportSnapshot`. `/data/guns_float?symbol=...` uses FMP `/stable/shares-float` directly, independent of Gateway connectivity, requiring `PAPER_FMP_KEY` in the Python server environment. Only actual matching-symbol float counts with provider dates less than 45 days old are accepted; successful responses cache for six hours. IB discovery and cheap/history stages run even without a float key; only survivors request float. Missing configuration still prevents complete publication, rather than invoking the retired IB fundamental API. Old complete, explicitly labeled outstanding-share-bound snapshots remain historical evidence, not newly acquired float. This float dependency is separate from the repaired IB bid/ask path.
...
**Optional alternative market data:** set `PAPER_ALPACA_KEY` and `PAPER_ALPACA_SECRET` (or `APCA_API_KEY_ID` / `APCA_API_SECRET_KEY`) in the Python server environment for Alpaca **real-time SIP** fallback. IBKR stays first for discovery, quotes, contract identity and sessions. If IB quote recovery fails, `/data/guns_quote?symbol=...` requests consolidated SIP bid/ask/trade with provider timestamps. If IB historical observations fail and keys are configured, verification requests SIP minute/daily bars with pagination. Free IEX and delayed SIP are not substitutes for consolidated real-time spread/volume. External quote snapshots are for screening only and never enter the paper-fill feed; execution still requires fresh IB quotes. No broker trading endpoint or paid regulatory snapshot is requested. `/data/guns_data_status` reports configuration booleans, never keys. Configuration is not a verified entitlement; no alternative keys were available in this sandbox.

**Research sources:** [IBKR 2026 release notes, 10.47](https://www.ibkrguides.com/releasenotes/prod-2026.htm); [IBKR price/size callback behavior](https://www.interactivebrokers.com/docs/tws-api/doc/market-data-live/top-of-book-l-1/receive-live-data); [Alpaca feed coverage](https://docs.alpaca.markets/docs/about-market-data-api); [SIP latest quotes](https://docs.alpaca.markets/reference/stocklatestquotes-1); [FMP shares-float](https://site.financialmodelingprep.com/developer/docs/stable/shares-float). The pinned library's `Wrapper.startTicker`, `IB.reqMktData` and `IB.cancelMktData` were inspected directly for the cancellation reproduction.
