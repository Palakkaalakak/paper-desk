...
- Added offline regressions for depth stream reuse, unchanged polling freshness, eviction, resets/reconnect generations, local credential authorization, atomic private file writes and UI key handling. The 60 Python and 59 JavaScript tests passed on 2026-09-15. Both existing browser suites passed with zero browser errors, including the FMP form and visible blockers. The additional off-tab Level II decision-control browser scenario is being checked separately; those earlier passes do not establish that new scenario or live Gateway readiness.
