...
const VERSION='guns-1.1',defaults={riskPct:1,rewardR:2,maxSpread:.05,minVolume:30000,breakeven:true,atrPeriod:14,stopMode:'ATR',fixedStop:.2};
...
function analyze(data,q,cfg,setup,notes,now){cfg=Object.assign({},defaults,cfg);notes=notes||{};now=now||Date.now();setup=Number(setup);const checks=[],errors=[],advisories=[];const advise=(label,ok)=>advisories.push({label,ok:!!ok});const check=(label,ok)=>{checks.push({label,ok:!!ok});if(!ok)errors.push(label);};
 ...
 let trigger=null,stop=setup<=2?null:undefined;const pre5=m5.filter(b=>sess&&b.t>=sess.start-19800000&&b.t<sess.start),basis=setup<=3?m5:closed;
 if(setup===1){trigger=pmHigh;advise('Within 5% of premarket high',pmHigh&&price>=pmHigh*.95&&price<=pmHigh*1.01);}
 if(setup===2){for(let i=1;i<pre5.length-1;i++)if(pre5[i].h>=pre5[i-1].h&&pre5[i].h>pre5[i+1].h&&pre5[i].h<pmHigh)trigger=pre5[i].h;advise('Suggested lower pivot identified',trigger!==null);}
 if(setup===3||setup===4){const f=flag(setup===3?pre5:regular);advise('Heuristic completed lower-high bull flag',!!f);if(f){trigger=f.candle.h;stop=f.candle.l-tick;advise('Estimated retracement no deeper than 60%',f.retracement<=.6);if(setup===4)advise('Heuristic first flag',f.first);const e=average(basis,20,true).at(-1);advise('Estimated flag holds 20 EMA',finite(e)&&f.low>=e);}}
 ...
 const human=notes.levels?.[setup],humanTrigger=setup<=4&&finite(human?.trigger)&&human.trigger>0;
 if(humanTrigger){trigger=human.trigger;if(setup>=3)stop=finite(human.candleLow)&&human.candleLow>0?human.candleLow-tick:undefined;}
 ...
 return {setup,checks,errors,advisories,levelSource:humanTrigger?'user':'suggested',entry,limit,stop,target,risk,atr:a,pmHigh,gap,volume,session:sess,
