...
const VERSION='guns-1.0',defaults={riskPct:1,rewardR:2,maxSpread:.05,minVolume:30000,breakeven:true,atrPeriod:14,stopMode:'ATR',fixedStop:.2};
...
function analyze(data,q,cfg,setup,notes,now){cfg=Object.assign({},defaults,cfg);notes=notes||{};now=now||Date.now();setup=Number(setup);const checks=[],errors=[];const check=(label,ok)=>{checks.push({label,ok:!!ok});if(!ok)errors.push(label);};
 if(!data)return {checks,errors:['Waiting for live broker chart data'],setup};
 ...
 check('Daily overhead resistance reviewed',notes.room===true);check('Chart stream current',finite(data.updatedAt)&&now-data.updatedAt<90000);
 ...
 let trigger=null,stop=null;const pre5=m5.filter(b=>sess&&b.t>=sess.start-19800000&&b.t<sess.start),basis=setup<=3?m5:closed;
 if(setup===1){trigger=pmHigh;check('Within 5% of premarket high',pmHigh&&price>=pmHigh*.95&&price<=pmHigh*1.01);}
 if(setup===2){for(let i=1;i<pre5.length-1;i++)if(pre5[i].h>=pre5[i-1].h&&pre5[i].h>pre5[i+1].h&&pre5[i].h<pmHigh)trigger=pre5[i].h;check('Lower pivot identified',trigger!==null);}
 if(setup===3||setup===4){const f=flag(setup===3?pre5:regular);check('Completed lower-high bull flag',!!f);if(f){trigger=f.candle.h;stop=f.candle.l-tick;check('Retracement no deeper than 60%',f.retracement<=.6);if(setup===4)check('First flag only',f.first);const e=average(basis,20,true).at(-1);check('Flag holds 20 EMA',finite(e)&&f.low>=e);}}
 if(setup===5){const first=regular[0];trigger=first?.h;stop=first?first.l-tick:null;check('First completed bullish 09:30 candle only',first&&sess&&first.t===sess.start&&first.c>first.o&&now<sess.start+120000);check('Candle <=2x premarket ATR (app guardrail)',first&&preAtr&&first.h-first.l<=2*preAtr);}
 ...
 if(finite(trigger)&&tick>0){entry=round(trigger+tick,tick,true);limit=round(entry+(entry<20?.03:.05),tick,true);if(stop===null){const dist=cfg.stopMode==='FIXED'?Number(cfg.fixedStop):cfg.stopMode==='PRICE'?(entry<20?.15:entry<30?.25:entry<50?.4:.5):a;if(finite(dist)&&dist>0)stop=entry-dist;}if(finite(stop)){stop=round(stop,tick,false);risk=entry-stop;if(risk>0)target=round(entry+risk*cfg.rewardR,tick,true);}}
 ...
 return {setup,checks,errors,entry,limit,stop,target,risk,atr:a,pmHigh,gap,volume,session:sess,
...
return {VERSION,defaults,names,day,round,average,studies,atr,aggregate,flag,analyze,size,exit};
});
