...
(function(r,f){if(typeof module==='object'&&module.exports)module.exports=f(require('./guns.js'));else r.GunsWorkflow=f(r.Guns);})(globalThis,function(C){'use strict';
 ...
 const SCANNER_CODE='TOP_PERC_GAIN';
 function providerRanked(row,at){return row?.scannerCode===SCANNER_CODE&&row.scannerSource==='IBKR Top % Gainers'&&Number.isInteger(row.rank)&&row.rank>=0&&row.rank<50&&Number.isFinite(row.scannerAt)&&row.scannerAt<=at&&at-row.scannerAt<1800000&&C.day(row.scannerAt)===C.day(at);}
 function candidate(row,q,evidence,ready,cfg,now){const why=quoteIssues(row,q,ready),e=evidence||{},finite=Number.isFinite;let pending=why.length>0;q=q?{...q,last:Object.hasOwn(q,'tradeLast')?q.tradeLast:q.last}:q;
  ...
  if(!providerRanked(row,now)){pending=true;why.push('Awaiting IBKR Top % Gainers ranking');}
  // Provider selection is authoritative. A missing/different local comparison never rejects it.
  ...
  const score=why.length?null:-row.rank;
 ...
 function rank(rows,quotes,evidence,ready,cfg,now){return rows.map(r=>candidate(r,quotes[r.conid],evidence.get(r.conid),ready(r.conid),cfg,now)).sort((a,b)=>Number(b.eligible)-Number(a.eligible)||(a.row.rank??999)-(b.row.rank??999));}
 ...
 function selectCandidates(pool,excluded,cfg){const skip=new Set(excluded.map(x=>Number(x.conid??x))),seen=new Set();return pool.filter(r=>!skip.has(Number(r.conid))&&!seen.has(Number(r.conid))&&seen.add(Number(r.conid))&&completeScreen(r,cfg)).sort((a,b)=>a.rank-b.rank).slice(0,4);}
 ...
 function screenSnapshot(c,at){return {...c.row,screen:{at,gap:c.gap,gapInfo:c.gapInfo,...c.closeEvidence,volume:c.volume,spread:c.spread,bid:c.bid,ask:c.ask,quoteAt:c.quoteAt,quoteSource:c.quoteSource,verificationSource:c.verificationSource,previousClose:c.previousClose,sessionDate:c.sessionDate,stockType:c.stockType,usListed:c.usListed,currency:c.currency,primaryExchange:c.primaryExchange,float:c.float,price:c.price,source:'IBKR Top % Gainers + verified market data',gapRole:'local-comparison-only'}};}
 // Invalid local comparisons display as unavailable, but never remove a provider-ranked stock.
 function comparisonGap(s){return Number.isFinite(s?.gap)&&s.previousCloseVerified===true&&s.previousCloseDate===s.expectedPreviousCloseDate&&s.previousCloseDate<s.sessionDate&&s.gapInfo?.previousClose===s.previousClose&&s.gapInfo.previousCloseDate===s.previousCloseDate&&Number.isFinite(s.gapInfo.price)&&Number.isFinite(s.previousClose)&&s.previousClose>0&&Math.abs(s.gap-100*(s.gapInfo.price-s.previousClose)/s.previousClose)<1e-8&&Number.isFinite(s.gapInfo.at)&&s.gapInfo.at<=s.at&&s.at-s.gapInfo.at<=60000?s.gap:null;}
 // Validate evidence at its screening time: saved cards are snapshots, not live reranks.
 function completeScreen(row,cfg){const s=row?.screen,finite=Number.isFinite;return !!(s&&finite(s.at)&&providerRanked(row,s.at)&&finite(s.price)&&s.price>=1.5&&finite(s.volume)&&s.volume>=cfg.minVolume&&finite(s.bid)&&s.bid>0&&finite(s.ask)&&s.ask>=s.bid&&finite(s.spread)&&s.spread>=0&&Math.abs(s.ask-s.bid-s.spread)<1e-8&&s.spread<=Math.min(cfg.maxSpread,.10)+1e-8&&finite(s.quoteAt)&&Math.abs(s.at-s.quoteAt)<15000&&s.stockType==='COMMON'&&s.usListed===true&&s.currency==='USD'&&typeof s.sessionDate==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(s.sessionDate)&&floatBelow(s.float,cfg.maxFloat||100000000,s.at));}
 ...
 function cheapFilter(items,stage,cfg){return items.filter(({q})=>{const last=Object.hasOwn(q||{},'tradeLast')?q.tradeLast:q?.last;if(stage==='gap')return true; // IBKR selects gainers; local gap never filters discovery.
if(stage==='price')return !(Number.isFinite(last)&&last<1.5);if(stage==='spread')return !(Number.isFinite(q?.bid)&&Number.isFinite(q?.ask)&&q.bid>0&&q.ask>=q.bid&&q.ask-q.bid>Math.min(cfg.maxSpread,.10)+1e-8);if(stage==='volume')return !(Number.isFinite(q?.volume)&&q.volume>=0&&q.volume<cfg.minVolume);return true;});}
 ...
 return {SCANNER_CODE,providerRanked,comparisonGap,defaults,chord,bindings,setBinding,candidate,rank,shortlist,scanDue,floatKnown,floatBelow,floatLabel,selectCandidates,stableSlots,screenSnapshot,completeScreen,quoteIssues,mapPool,cheapFilter,storyKey,sameStory};
});
