...
(function(r,f){if(typeof module==='object'&&module.exports)module.exports=f();else r.GunsWorkflow=f();})(globalThis,function(){'use strict';
 ...
 async function mapPool(items,limit,work,options={}){const results=Array(items.length),ctl=new AbortController();let next=0,active=0,done=0,timer,finish;const stopped=new Promise(resolve=>finish=resolve),abort=()=>{ctl.abort();finish();};options.signal?.addEventListener('abort',abort,{once:true});if(options.signal?.aborted)abort();if(Number.isFinite(options.timeoutMs)){if(options.timeoutMs<=0)abort();else timer=setTimeout(abort,options.timeoutMs);}const report=()=>options.onProgress?.({active,done,total:items.length});async function worker(){while(next<items.length&&!ctl.signal.aborted){const i=next++;active++;report();let result;try{result={value:await work(items[i],i,ctl.signal)};}catch(error){result={error};}active--;if(ctl.signal.aborted)return;results[i]=result;done++;report();}}try{await Promise.race([Promise.all(Array.from({length:Math.min(items.length,Math.max(1,Math.min(8,limit)))},worker)),stopped]);return Array.from({length:items.length},(_,i)=>results[i]||{error:Object.assign(Error('Deadline reached; not verified'),{code:'deadline'})});}finally{clearTimeout(timer);options.signal?.removeEventListener('abort',abort);ctl.abort();}}
 function cheapFilter(items,stage,cfg){return items.filter(({q})=>{const last=Object.hasOwn(q||{},'tradeLast')?q.tradeLast:q?.last;if(stage==='gap'){const close=q?.prevClose;return !(Number.isFinite(last)&&Number.isFinite(close)&&close>0&&100*(last/close-1)<5);}if(stage==='price')return !(Number.isFinite(last)&&last<1.5);if(stage==='spread')return !(Number.isFinite(q?.bid)&&Number.isFinite(q?.ask)&&q.bid>0&&q.ask>=q.bid&&q.ask-q.bid>Math.min(cfg.maxSpread,.10)+1e-8);if(stage==='volume')return !(Number.isFinite(q?.volume)&&q.volume>=0&&q.volume<cfg.minVolume);return true;});}
 ...
 return {defaults,chord,bindings,setBinding,candidate,rank,shortlist,scanDue,floatKnown,floatBelow,floatLabel,stableSlots,screenSnapshot,completeScreen,quoteIssues,mapPool,cheapFilter,storyKey,sameStory};
});
