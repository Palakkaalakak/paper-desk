...
(function(r,f){if(typeof module==='object'&&module.exports)module.exports=f();else r.GunsWorkflow=f();})(globalThis,function(){'use strict';
 ...
 async function mapPool(items,limit,work){const results=Array(items.length);let next=0;async function worker(){while(next<items.length){const i=next++;try{results[i]={value:await work(items[i],i)};}catch(error){results[i]={error};}}}await Promise.all(Array.from({length:Math.min(items.length,Math.max(1,Math.min(8,limit)))},worker));return results;}
